# Evidence — What the App Does (Plain-Language Requirements, v4)

This document describes everything in the app in plain English.
**To request changes: edit anything below — cross things out, rewrite wording,
add options, reorder lists — and send it back.** The app will be updated to
match.

---

## 1. Home screen

A big green button: **I NEED INTENSITY**

Below it, five smaller buttons:

- SHOW ME EVIDENCE I'M CHANGING
- LOG A WIN
- GRATITUDE
- IMMEDIATE CONNECTION
- BIG DECISIONS

If a check-in was started but not finished, a banner appears at the top:
"You have an unfinished check-in — pick up where you left off — nothing was
lost."

At the bottom, the core promise: *You do not have to decide right now. Pause.
Notice. Choose one next step. Collect evidence.*

---

## 2. The "I Need Intensity" check-in

Four quick screens, in this order. Every screen has a back arrow, and
**every screen saves automatically** — closing the app mid-check-in loses
nothing; it resumes exactly where you left off.

### Screen 1 — What do you want to do right now? (pick up to three)

get high · spend money · sleep · orgasm · flirt · human touch ·
text someone · write · give in on a boundary · disappear · don't know

### Screen 2 — What is happening in your body? (pick any)

adrenaline pumping · stomach pain · throw up · buzzing · numb · empty · obsessive thinking /
ruminating · eye twitching · tight chest · racing thoughts · can't sit
still · exhausted · pressure in body · knot in stomach · heavy body ·
don't know

### Screen 3 — How are you feeling? (pick any)

bored · lonely · overwhelmed · angry · aroused · sad · grief · stuck ·
excited · activated · ashamed · anxious · rejected · disappointed ·
I worked hard — I deserve to take it easy · I want to feel good ·
I want to escape · urge to fix something · urge to use · don't know

### Screen 4 — What else is going on? (pick any)

**Physical:** hungry · thirsty · tired · sick · pain · haven't moved my
body · forgot medication · menstruating

**Life Stressors:** kids · co-parent · ex · work · money · parents ·
adulting · decision fatigue · uncertainty

---

## 3. The pause screen

After the check-in, the app says:

> You do not need to understand this yet.
> You only need to pause.
> Try one thing for 10 minutes.

If "give in on a boundary" was selected, this line also appears:
*"I can feel how much she hates this — and how much I hate her being
upset — without rescuing either of us from it."*

Then ONE big green **TRY THIS** button.

---

## 4. The suggestions (Try one thing)

Organized by time, not by feeling. One flat list — pick anything.

**Quick — even while working:** sing · dance · 3 breaths · meditate ·
Sunny snuggles · text a friend · list 3 gratitudes · text my coach ·
sour candy · suck on a lollipop · drink sparkling water · drink through a
straw · chew gum · prayer · write morning pages · look at photos from
this day last year · review evidence · compare today to six months ago ·
read wins

**If you have more time:** play bass · lift weights · cold shower · hot
shower · walk Sunny · go outside · lie down · take a nap · fast walk ·
bike ride · stomp · hit pillow or punching bag · 15-minute sober writing
sprint · call my sponsor · spicy food

Also on this screen: a **Start a 10-minute timer** button, the line "You
can do anything for 10 minutes," and a note that everything is saved — if
you close the app, you can finish the reflection later (the home screen
will remind you).

---

## 5. The reflection screen (after trying something)

At the top, a permission card that names the feelings selected earlier:

> **Permission is not action.**
> I give myself permission to feel [angry, aroused, ...] — without acting
> on it. These are human feelings. I'm empowered to choose how I navigate
> them. I don't have to fall back on my numbing or escape patterns.

**What happened?** (pick any): craving passed · craving decreased · craving
stayed · I acted on it · I paused · I reached out · I learned something ·
I still don't know

If "I acted on it" is selected, the full no-shame message appears ("No
shame spiral. Not tonight." ...).

**Evidence that I'm changing** — tap any that are true (all selections are
kept), plus a box to write your own:

I paused. · I waited 10 minutes. · Craving survived. · I told the truth. ·
I texted Anthony. · Asked for help. · I noticed the pattern. · Delayed a
big decision. · I let myself feel without acting. · I came back. ·
Boundary held. · Asked for what I needed. · Parenting win. · Recovery
win. · I apologized. · Honest conversation. · Attended a meeting. · Made
amends. · Chose repair over avoidance. · Finished a hard
task. · Took medication. · Exercised. · Ate a meal. · Chose rest. · Stayed
present with discomfort. · Followed through on a commitment. · Did
something sober that I thought I needed a substance to do. · Let someone
influence me without losing myself.

---

## 6. Evidence dashboard ("Proof I'm Changing")

Shows: total pieces of evidence, number of check-ins, counts in each
category, your most common dysregulators, and the most recent entries.
**Tapping a category opens it to show the entries behind the number.**
Every entry can be edited or deleted.

Categories counted: Pauses · Gratitudes · Support reach-outs · Cravings
survived · Honest moments · Parenting wins · Recovery wins · Boundaries
held · Difficult conversations · Meetings attended · Acts of repair ·
Sober creative sessions · Decisions delayed · Other evidence

No streaks, ever. Evidence over perfection.

At the bottom: a link to Reports, and "Erase all data on this device"
(for clearing test entries).

---

## 7. Reports

- **Report builder:** "After I [event], how often did I [event] within
  [time window]?" — combining wins, gratitudes, check-ins, reach-outs,
  pauses, and delayed decisions.
- **What follows each path?** How often a **win** (wins only — gratitudes
  are not the measure of success) followed within a day of check-ins where
  you rode it out vs. acted on it. Never framed as a scoreboard.
- **My most helpful actions:** your top 10 specific actions (not
  categories), ranked by how often the craving passed or eased after
  trying them.
- **When do cravings hit?** By time of day and day of week.
- **What shows up before I cave?** Dysregulators and feelings from
  check-ins that ended with "I acted on it."
- **What do I reach for?** Most common urges.

---

## 8. Immediate Connection screen

- **Anthony** — Text / Call (781-706-4853)
- **Cheryl** — Text / Call (336-404-5959)
- **Trusted Friend** — Call (number can be added in the app)
- **Open meeting list** (currently links to aa.org's meeting finder)
- **Pray** — shows: *Help me be honest. Help me pause. Help me choose the
  next right thing.*

Tapping text/call on a contact automatically logs "Reached out" as evidence.

---

## 9. Log a Win

A box to write any win, with the same one-tap example list as the
reflection screen, a category picker, and a date picker so a win from an
earlier day can be logged on the day it happened. Any size counts. What
you type is saved as you go — a half-written win survives closing the app.

---

## 10. Gratitude

A quick page: type anything you're thankful for and save it — as many as
you want in a row. Each is saved as evidence with date and time, and the
most recent ten are shown.

---

## 11. Big Decisions

For tattoos, purchases, relationships, job changes, quitting, starting.

1. "What is the decision?" (write it down)
2. "Have I already decided?" — yes / maybe / no
   - **Yes** → process it: What does this move me toward? What does it help
     me avoid? Who will this affect? What would waiting 24 hours cost?
   - **Maybe** → explore it: What am I hoping this changes? What need is
     underneath? What's the smallest version I could try first? Whose voice
     do I trust on this?
   - **No** → evaluate it: What are the real options, including "not yet"?
     What would future me thank me for? What evidence would help me decide?
     What happens if I wait a week?
3. Always shows: **"Am I protecting my autonomy? Or protecting access to a
   behavior?"**
4. Buttons: "Delay this decision for now" (counts as evidence) · "Save and
   step away" · "I'm activated — regulate first" (goes to the check-in)

---

## 12. Rules the app always follows

1. **No shame.** Never "you failed / you relapsed." Always "you noticed /
   you paused / you came back / evidence counts."
2. **Regulation before analysis.** No insight required to use the app.
3. **Permission is not action.**
4. **Evidence over perfection.** No streaks.
5. **Nothing gets lost.** Every screen saves as you go.
6. **Privacy:** everything logged stays on the phone it was logged on.
   Nothing is uploaded anywhere.
